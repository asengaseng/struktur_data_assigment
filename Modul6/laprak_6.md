...
Bahasa C merupakan bahasa pemrograman terstruktur yang mengorganisir kode ke dalam blok-blok tertentu guna mempermudah proses pengembangan dan pemeliharaan program. Sebagai pengembangan dari konsep tersebut, kode sumber C++ umumnya disimpan dalam berkas berekstensi .cpp. Namun, instruksi yang ditulis tidak dapat langsung dijalankan oleh komputer; berkas tersebut harus terlebih dahulu melewati proses kompilasi menggunakan kompiler C++ agar dapat diubah menjadi format yang siap dieksekusi.

#### A. Doubly Linked List (DLL)
Doubly Linked List merupakan pengembangan dari linked list di mana setiap elemen (node) memiliki keterhubungan dua arah. Berbeda dengan singly linked list yang hanya bisa bergerak maju, setiap elemen pada doubly linked list dibekali dengan dua penunjuk atau successor: satu untuk mengakses elemen berikutnya (Next) dan satu lagi untuk kembali ke elemen sebelumnya (Prev).

Untuk mengelola rangkaian data ini, terdapat dua penunjuk utama sebagai kendali list:

First: Pointer yang mengunci lokasi elemen paling awal di dalam list.
Last: Pointer yang mengunci lokasi elemen paling akhir di dalam list.

#### 1. Searching
Searching adalah salah satu operasi fundamental dalam manajemen list yang bertujuan untuk menemukan node tertentu di dalam rangkaian data. Secara teknis, proses ini bekerja dengan metode penelusuran satu per satu (traversal), di mana program mengunjungi setiap elemen secara berurutan mulai dari awal dan akan berhenti seketika saat data yang diinginkan telah ditemukan.
...
```
program ini berfungsi untuk menampilkan hasil pembaruan, kemudian mencari data menggunakan fungsi searchByName, serta menghapus data baik secara umum, berdasarkan data pertama, maupun berdasarkan data sebelumnya (dataBefore).
...
### 1. 
### Output
![Screenshot Output 2](https://github.com/asengaseng/struktur_data_assigment/blob/main/Modul6/Output-6.png)
https://github.com/asengaseng/struktur_data_assigment/blob/main/Modul6/Output-6.png.png 
...
## Kesimpulan
Pembelajaran ini menonjolkan efisiensi Doubly Linked List (DLL) yang terletak pada kemampuan navigasi dua arah berkat adanya pointer next dan prev pada setiap node. Keunggulan utama yang terlihat adalah fleksibilitas struktur ini saat dimodifikasi untuk menangani kasus spesifik, seperti pengelolaan data duplikat (Nomor Polisi). Dengan mengoptimalkan fungsi pencarian dan penghapusan menjadi searchAndPrintAll serta deleteAllMatches, program membuktikan bahwa DLL sangat adaptif untuk operasi manipulasi data di posisi mana pun dengan efisiensi yang terjaga.
